...
**实现方式：Bot 对话驱动 + 逐 Skill 调用**

```
用户: "我要逐步协作完成复审"
  │
  ▼
Bot: "好的，我们将逐步完成复审流程。当前第一步：审查意见解析。
...
Bot: [调用 opinion-analysis Skill] 
  → 输出驳回理由清单
  "解析完成。共识别 3 条驳回理由：
  RG-1: 权利要求1相对于D1不具备新颖性（专利法§22.2）
  RG-2: 权利要求2-3相对于D1+D2不具备创造性（专利法§22.3）
  RG-3: 权利要求4不清楚（专利法§26.4）
  
  是否有需要修改的地方？输入'确认'继续下一步，或输入修改意见。"

用户: "RG-1的引用文献公开号好像不对，应该是CN108123456A"

Bot: "已记录你的修改。驳回理由 RG-1 的引用文献公开号已更新为 CN108123456A。
  下一步：答辩理由映射。是否开始？"
```
...
db.execute("INSERT INTO claim_chart_results (case_id, claim_number, features, warnings) VALUES (?, ?, ?, ?)",
           [case_id, claim_number, features_json, warnings_json])
