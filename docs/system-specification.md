...
> **变更摘要（v0.10.0）**：依据第 9 轮审查修复 2 项——
> 1. **新增附录 G：Coze.cn 移植 Skill 架构指导**：18 个模块到 Coze.cn Skill 的映射、独立运行输入要求、Skill 间依赖链（DAG）、移植注意事项（backlog 62）
> 2. **模块 11 `examinerResponse` 警告措辞强化**：从"建议代码层同步删除"改为"Coze.cn 移植时禁止包含"
>
> 本文档对 `patentExaminator` (https://github.com/wukun2005-gif/patentExaminator/) 项目进行完整扫描，提取核心业务逻辑、数据流转（Schema）以及 AI 提示词（Prompts），以便无缝移植到 [Coze.cn](https://www.coze.cn/) 低代码 Agent 开发平台。
>
...
27. [附录 G：Coze.cn 移植 Skill 架构指导](#27-附录-gcozecn-移植-skill-架构指导)

---

## 0. 场景和基础LLM

用户所处的复审场景是有明确的业务流的，但是这个assistant目的不是替代她现有工作中的业务流系统，而是支持她提供工作效率，虽然越贴近她的工作流越好，但是没必要做一个和她工作中业务流系统一样和类似的系统，她不会用的，也没有意义，但是assistant越贴近她的工作流当然用起来越顺手；她提出的问题有可能是工作流环节base的开发性问题，现有的单位工作流系统无法有效或者高效或者根本无法回答，比如，她在单位工作流系统里面看到新的专利申请内容，可能根本不知道这个专利在说什么，因为专利讲述的是一个全新的科技领域的世界尖端科技知识，比如量子纠缠。因此她需要到处查资料了解量子纠缠的基本原理，这些专利解读和科普内容在她单位现有的工作流系统里面完全没有，所有审查员都要自己想办法解决这些问题。类似的业务流程中环节里面全靠人肉的地方还有很多。我的这个app/agent/bot旨在通过AI来辅助解决这些环节中人肉的部分来提高效率。

审查员的三种基本操作场景：

### 0.1 一键直出：审查员一次性上传所有的复审文件，app/agent/bot 借助AI自动走完所有复审流程要求的所有环节，无需审查员中间介入与交互，直接给出AI生成的复审意见草稿，供审查员参考修改。审查员可以回溯复审流程要求的所有环节，查看每个环节AI的生成结果，并在每个环节做人工调整和再生成，来调试最终的复审意见草稿。

### 0.2 每步协作：审查员一次性上传所有的复审文件，app/agent/bot 借助AI逐步和审查员一步一步走完所有复审流程要求的所有环节，每个环节都需要审查员介入与交互，生成每个环节的内容，供审查员参考修改。审查员可以回溯复审流程要求的所有环节，查看每个环节AI的生成结果，并在每个环节做人工调整和再生成，直到走到最后一步生成 AI和审查员共同协作完成的复审意见草稿。

### 0.3 部分协作：审查员一次性上传所有的复审文件，app/agent/bot 借助AI在审查员选择和要求的部分环节（如果是0环节，就是 “0.1 一键直出”场景；如果是全部环节，就是“0.2 每步协作”场景）和审查员协作，生成这些环节的内容，供审查员参考修改。比如审查员审查员可以选择“文档解读”环节和 app/agent/bot共同协作，其他环节让 app/agent/bot 全自动走完其他所有环节。审查员可以回溯复审流程要求的所有环节，查看每个环节AI的生成结果，并在每个环节做人工调整和再生成，直到走到最后一步生成 AI和审查员共同协作完成的复审意见草稿。

我选用的模型是XiaoMi MiMo系列：
- mimo-v2.5-pro：专攻极难的逻辑推理、长文本一致性和代码生成，推理能力对标 Claude Opus 4.6，但无视觉能力。
- mimo-v2.5：原生全模态大模型，拥有极强的图片、图表、OCR 识别能力。

图纸通常都在申请书和权利要求书里面。专利文件通常是一个长达数十页的单一 PDF，权利要求书、说明书正文在前，而“说明书附图”被打包在 PDF 的最后几页。
...
// resolveAgent() 行为说明（b24/b25）：
// - providerId 取 providerOrder[0]（去硬编码，不再映射到固定 Provider）
// - modelId 自动跟随 Provider 的 defaultModelId（b24）：
//   const providerSetting = providerSettings.find(p => p.providerId === providerId);
//   modelId = providerSetting?.defaultModelId ?? assignment.modelId;
// - maxTokens 直接从 assignment.maxTokens 读取
...
interface SearchSession {
  ...
  updatedAt: string;  // ISODateTimeString
}

// Provider Key 同步结果（bg-39 新增）
// 来源：client/src/lib/repositories/settingsRepo.ts（非 shared 包）
...
### IndexedDB Store 清单（bg-43 补全）

`clearAllLocalData()` 清除以下全部 IDB store：
...
> **新增 store**（bg-43）：`runMarkers`（Agent 运行标记）和 `searchSessions`（检索会话持久化）此前遗漏，已补全。
>
...
本模块为纯前端处理（PDF 文字层检测、OCR、文本索引建立），不涉及 AI 调用，无 Prompt 模板。
...
```json
{
  ...
  "rejectionGrounds": [
    {
      "code": "string（如 RG-1、RG-2）",
      "category": "\"novelty\" | \"inventive\" | \"clarity\" | \"support\" | \"amendment\" | \"other\"",
      "claimNumbers": "[number]",
      "summary": "string（50字以内）",
      "legalBasis": "string（如 '专利法第22条第2款'）",
      "originalText": "string | undefined"
    }
  ],
  "citedReferences": [
    {
      "publicationNumber": "string（如 CN108123456A）",
...
> `rejectionGrounds` 允许空数组（无驳回理由时返回 `[]`）。
...
```json
{
  "mappings": [
    {
      "rejectionGroundCode": "string（驳回理由 code，如 RG-1）",
      "applicantArgument": "string（申请人答辩原文片段）",
      "argumentSummary": "string（答辩理由摘要，50字以内）",
      "confidence": "\"high\" | \"medium\" | \"low\"",
      "amendedClaims": [
        {
          "claimNumber": "number",
  ...
  "unmappedGrounds": "string[] | undefined（未在意见陈述书中找到对应答辩的驳回理由 code 数组）",
  "legalCaution": "string"
}
```

> **Zod 容错说明**：`amendedClaims` 中的每个条目使用 `z.union([object, string])` 兼容 AI 返回字符串而非对象的降级处理——当 AI 返回字符串时自动转为 `{ claimNumber: 0, originalText: "", amendedText: "", changeDescription: s }`。Coze.cn 低代码平台的输出格式化能力有限，移植时需注意此兼容逻辑。
...
- 模块 2（意见陈述书文本已提取）
- 模块 3（文件已分类为 office-action-response）
...
本模块为纯前端处理（时间轴校验），不涉及 AI 调用，无 Prompt 模板。
...
```json
{
  ...
  "warnings": "string[]",
  "pendingSearchQuestions": "string[]（最多 5 条）",
  "legalCaution": "string"
}
```
  ...
  "rows": [
    {
      ...
      "mismatchNotes": "string | undefined",
      "reviewerNotes": "string | undefined"
    }
  ],
  "differenceFeatureCodes": "string[]",
  "pendingSearchQuestions": "string[]（最多 5 条）",
  "legalCaution": "string（默认 '以上为候选事实整理，不构成新颖性法律结论。'）"
}
```

> `rows` 至少 1 项。
>
> **注意**：`reviewerConclusions` 和 `aiPreliminaryConclusions` 不在 AI 返回 Schema 中——它们属于前端 storage 层的扩展字段（`NoveltyComparison` 类型），用于存储用户的编辑内容。Coze.cn 移植时仅需关注 AI 返回 Schema。
...
{
  "claimNumber": "number（正整数）",
  "closestPriorArtId": "string | undefined（最接近现有技术的 referenceId）",
  "sharedFeatureCodes": "string[]（共有特征的 featureCode 数组）",
  "distinguishingFeatureCodes": "string[]（区别特征的 featureCode 数组）",
  "objectiveTechnicalProblem": "string | undefined",
  "motivationEvidence": [
    {
      "referenceId": "string",
      "label": "string",
      "paragraph": "string | number | null | undefined（容错：AI 可能返回数字）",
      "quote": "string | undefined",
      "confidence": "\"high\" | \"medium\" | \"low\""
    }
  ],
  "candidateAssessment": "\"possibly-lacks-inventiveness\" | \"possibly-inventive\" | \"insufficient-evidence\" | \"not-analyzed\"",
  "cautions": "string[]",
  "legalCaution": "string（默认 '以上为候选事实整理，不构成创造性法律结论。'）"
}
```

> **Zod 容错说明**：`paragraph` 字段使用 `z.union([z.string(), z.number(), z.null()])` 兼容 AI 可能返回数字（如段落号 5 而非 "0005"），通过 `.transform()` 统一转为字符串。`motivationEvidence` 默认空数组 `[]`，`candidateAssessment` 默认 `"not-analyzed"`。
...
- **注意**：本文档 Prompt 模板已移除 `examinerResponse` 字段（Zod Schema 未定义，会被静默丢弃），但代码 `AgentClient.ts` 的 `buildInventivePrompt()` 函数中仍保留该字段（Prompt JSON 示例 + 返回对象）。**Coze.cn 移植时禁止包含 `examinerResponse` 字段**——该字段每次调用浪费约 50 token 生成无用文本。代码层遗留问题待后续清理。
...
```json
{
  "defects": [
    {
      "category": "string（权利要求、说明书、附图、摘要、其他）",
      "description": "string（非空）",
      "location": "string | undefined",
      "severity": "\"error\" | \"warning\" | \"info\"",
      "previouslyRaised": "boolean | undefined",
      "overcomeStatus": "\"overcome\" | \"not-overcome\" | \"partially-overcome\" | undefined"
    }
  ],
  "warnings": "string[]",
  "legalCaution": "string"
...
```json
{
  ...
  "defectReviewSummary": "string | undefined",
  "legalCaution": "string"
}
```
...
| 输出示例 | 含 `examinerResponse` 字段的完整示例 | 同样含 `examinerResponse`（⚠️ Zod Schema 未定义该字段，AI 输出被静默丢弃——**Coze.cn 移植时禁止包含此字段**，见模块 11 业务规则） |
...
```json
{
  "body": "string（简述正文：技术方案概述 + 审查意见核心结论）",
  "aiNotes": "string（AI 备注：不确定内容、citation 不满足门禁的内容）",
  "legalCaution": "string"
}
```
...
从权利要求中提取检索要素（关键词），允许审查员编辑检索词后，调用外部专利搜索引擎（Tavily / SerpAPI / EPO OPS），再用 AI 筛选和排序搜索结果，输出候选项供审查员添加为对比文件。

**两步式流程（nf-7）**：检索流程拆分为两个独立步骤——
1. **提取检索词**（`/extract-search-terms`）：AI 从权利要求中提取 3-5 条检索词，返回给用户
2. **执行搜索**（`/search-with-terms`）：用户编辑确认检索词后，调用搜索引擎 + AI 筛选

此设计允许审查员在搜索前修正检索词，提高检索精准度。检索词和 Provider 结果通过 `SearchSession` 持久化到 IndexedDB，页面刷新后可恢复。

### 检索状态机（SearchStep）

```typescript
type SearchStep = "idle" | "extracting" | "editing" | "searching" | "done";
```

```
idle → extracting（点击"提取检索词"）
  → editing（检索词提取完成，展示可编辑列表）
    → searching（点击"执行搜索"）
      → done（搜索完成，展示结果 + Provider 计数）
    → editing（搜索失败，回到编辑态允许重试）
```

### Step 1：提取检索词

#### 输入 Schema（`ExtractSearchTermsRequest`）

```json
{
  "caseId": "string",
  "claimText": "string（权利要求全文）",
  "features": [
    { "featureCode": "string", "description": "string" }
  ],
...
#### Server 端点：`POST /extract-search-terms`

```typescript
// server/src/routes/search.ts — extractTermsSchema
{
  caseId: z.string(),
...
#### AI 返回 Schema（`ExtractSearchTermsResponse`）

```json
{
  "ok": "boolean",
  "queries": "string[]（提取的检索词列表，3-5 条）",
  "featureCount": "number（输入的技术特征数量）",
...
> **实际发送方式**：代码中以单条 `role: "user"` message 发送，角色身份与指令合并。
...
> **截断限制**：`claimText.slice(0, 4000)`，生成的检索式最多 5 条（`Array.slice(0, 5)`）。
...
Server 端对 AI 返回的检索词执行以下后处理：
1. 优先从 JSON 中提取 `queries` 数组；若 JSON 解析失败，按行拆分并清理前缀（`/^[-•*\d.)`\s]+/`）
2. 过滤条件：长度 ≥ 3、排除 markdown fence（`` ``` ``）和 JSON 开头（`{`/`[`）
3. 截断到最多 5 条
4. 若结果为空，使用原始 AI 输出文本作为兜底

### Step 2：执行搜索（用户编辑后）

#### 输入 Schema（`SearchWithTermsRequest`）
...
```json
{
  ...
  "searchQueries": "string[]（用户编辑后的检索词，至少 1 条）",
  "maxResults": "number | undefined（1-10，默认 5）",
  "searchProviderId": "string | undefined",
  "searchApiKey": "string | undefined",
  "searchBaseUrl": "string | undefined"
}
...
#### Server 端点：`POST /search-with-terms`

```typescript
// server/src/routes/search.ts — searchWithTermsSchema
{
  caseId: z.string(),
  claimText: z.string().min(1),
  features: z.array(z.object({ featureCode: z.string(), description: z.string() })),
  searchQueries: z.array(z.string().min(1)).min(1),  // 用户编辑后的检索词，至少 1 条
...
#### AI 返回 Schema（`SearchReferencesResponse`）
...
```json
{
  ...
  "searchQuery": "string | undefined（@deprecated，使用 searchSummary 代替）",
  "searchSummary": {
    "featureCount": "number",
    "queryCount": "number",
    "dataSource": "string",
    ...
    "providerResults": [
      {
        "providerId": "string",
        "providerName": "string",
        "resultCount": "number（搜索引擎返回的原始结果数）",
        "candidateCount": "number（AI 筛选后的候选文献数）"
      }
    ]
  },
  "error": "string | undefined"
...
步骤 2 包含两个子步骤：EPO 翻译（条件触发）+ 结果筛选。

**子步骤 2a：EPO 中文转英文**（条件触发）

> **触发条件**：`searchProviderId === "epo"` 且检索式中存在中文字符（`/[一-鿿]/`）。
...
任务：从搜索结果中识别专利文献，提取 title, publicationNumber, publicationDate, summary, relevanceScore, recommendationReason, sourceUrl。按相关度排序，最多返回{maxResults}篇。

输出 JSON 数组格式。
```

> **截断限制**：步骤 2b 中 `claimText.slice(0, 2000)`。
...
### 前端 UI 流程（`ReferenceSearchPanel.tsx`）

1. **idle 态**：展示"提取检索词"按钮
2. **extracting 态**：按钮禁用，显示加载动画
3. **editing 态**：展示可编辑检索词列表（每条可修改/删除/新增），底部"执行搜索"按钮
4. **searching 态**：按钮禁用，显示加载动画
5. **done 态**：展示候选文献列表 + 每个 Provider 的结果计数（原始结果数 / 候选数），检索词仍可编辑后重新搜索

**并行多 Provider 搜索**：前端对所有已启用的搜索 Provider 并行发起请求（`Promise.all`），合并去重后取 top N。每个 Provider 的结果计数单独展示。

### 检索会话持久化（`SearchSession`）
...
- **存储**：IndexedDB `searchSessions` store（`searchSessionRepo.ts`），按 `caseId` 建索引
- **恢复**：页面加载时自动恢复最近一次检索会话（`getLatestSearchSession(caseId)`）
- **更新**：搜索完成后创建/更新 SearchSession

> **与 prompt 文件的差异**：`shared/src/prompts/search-references.prompt.md` 与代码 `server/src/routes/search.ts` 的对比如下，以代码为准：

| 差异类别 | prompt 文件 | 代码实际 |
|---------|------------|---------|
| 文档定位 | 设计稿文档，含"用途一：提取检索关键词"和"用途二：筛选搜索结果"两部分 | 实际实现分为 2 个独立端点（提取、搜索），每端点内含子步骤 |
| 流程设计 | 单步式（提取 + 搜索一体化） | 两步式（nf-7）：提取检索词 → 用户编辑 → 执行搜索 |
| 步骤 1 角色 | "你是一名专利检索分析专家" | "你是资深专利检索专家" |
| 步骤 1 内容 | 简述 `searchTerms`/`ipcCodes`/`searchQuery` 输出字段 | 含 5 条详细检索策略要求 + 2 个示例（LED 散热、锂电池快充） |
| 步骤 1.5（EPO 翻译） | 未单独列出 | 独立子步骤，仅 `searchProviderId === "epo"` 且含中文时触发 |
| 步骤 2 输出 | JSON 含 `candidates` + `searchQuery` + `legalCaution` | JSON 为纯数组 `[...]`，无外层包装 |
| 硬约束 | 4 条（绝不编造文献、不输出法律结论、公开号必须有据、相关度基于实际内容） | 内嵌在步骤 2b 的 Prompt 中（从搜索结果中识别专利文献、提取结构化字段） |
| 持久化 | 无 | `SearchSession` 持久化到 IndexedDB，支持页面刷新恢复 |

### 前置依赖

- 模块 9（Claim Chart 已完成，提供技术特征）
- 步骤 2 仅限真实模式运行（需配置外部搜索 API Key）

### 业务规则与备注

- 检索结果需审查员逐篇确认，确认后可作为对比文件导入
- 候选项状态三态：pending / accepted / rejected
- 步骤 1（提取检索词）在 mock 模式下返回固定示例词
- 步骤 2（执行搜索）在 mock 模式下返回模拟候选文献
- EPO 翻译失败时降级使用原始中文检索式
- 对比文件数量上限：10 篇（`MAX_REFERENCES`）
...
```json
{
  "caseId": "string",
  "sessionId": "string",
  "moduleScope": "\"claim-chart\" | \"novelty\" | \"inventive\" | \"summary\" | \"draft\" | \"defects\" | \"case\" | \"documents\" | \"interpret\" | \"opinion-analysis\" | \"argument-mapping\"",
  "userMessage": "string",
  "contextSummary": "string（由 chatContext.ts 自动构建的模块上下文摘要）",
  "history": [
...
- `reply`：AI 对话回复文本
- `action`（可选，未触发操作时不返回此字段）：当用户输入包含"重新 + 目标模块"关键词时，返回操作指令，前端展示"应用修改"按钮触发重跑
...
```json
{
  "caseData": "PatentCase（案件基线信息）",
  "claimFeatures": "ClaimFeature[]（已确认的技术特征列表）",
  "noveltyComparisons": "NoveltyComparison[]（新颖性对照结果）",
  "differenceFeatureCodes": "string[]（区别特征候选代码列表）",
  "pendingSearchQuestions": "string[]（待检索问题清单）",
  "inventiveAnalysis": "InventiveStepAnalysis | undefined（创造性三步法分析结果）",
  "defects": "FormalDefect[] | undefined（形式缺陷清单）",
  "reexamDraft": "ReexamDraftResponse | undefined（复审意见草稿）",
  "summary": "SummaryResponse | undefined（审查意见简述）"
}
```
...
- **XSS 防护**（bg-32）：`escapeHtml()` 函数对所有用户生成内容（案件信息、特征描述、引文、审查结论等）执行 HTML 实体转义（`&` → `&amp;`、`<` → `&lt;`、`>` → `&gt;`、`"` → `&quot;`、`'` → `&#039;`），防止注入攻击
...
```json
{
  "translatedText": "string（翻译后的中文文本，纯字符串，非结构化 JSON）"
}
```
...
- **`POST /api/extract-search-terms`**：两步式检索 Step 1——仅提取检索词（`server/src/routes/search.ts`）
- **`POST /api/search-with-terms`**：两步式检索 Step 2——用用户编辑后的检索词执行搜索（`server/src/routes/search.ts`）
- **`POST /api/search-references`**：旧版一体化检索入口（仍保留，但前端已切换到两步式流程）
...
// server/src/lib/schemas.ts
{
  ...
  maxTokens?: number,                    // 最大 token 数，默认 4096（b23 新增：可选字段）
...
推理模型（reasoning model）自动扩容 maxTokens，避免长思考链截断：
...
function resolveMaxTokens(modelId: string, requestedMaxTokens?: number): number {
  const base = requestedMaxTokens ?? 4096;
  if (REASONING_MODEL_PATTERNS.test(modelId)) {
    return base * REASONING_MAX_TOKENS_MULTIPLIER;  // 4x: 4096 → 16384
  }
  return base;
}
```

- **触发条件**：模型 ID 匹配 `/mimo|r1\b|o[134]\b|reasoner|thinking/i`（MiMo、DeepSeek R1、OpenAI o1/o3 系列等）
- **扩容倍率**：4x（基础 4096 → 16384）
- **生效范围**：所有 Provider Adapter（OpenAICompatible、Bedrock、Gemini）的 `chat()` 方法均调用 `resolveMaxTokens()`
- **AgentClient 传递**：`resolveAgent()` 返回的 `maxTokens` 从 `AgentAssignment.maxTokens` 读取，由前端配置决定
...
5. **Provider 选择与回退**（`registry.runWithFallback`）：
   ...
   - 中间重试尝试记录到 `attempts` 数组（bg-28 fix），每个条目含 `{ providerId, ok, errorCode? }`
...
1. **剥离 markdown fence**：移除 ` ```json ` / ` ``` ` 包裹（`stripCodeFences`）
2. **全文直接解析**：尝试 `JSON.parse(cleaned)`
3. **平衡括号提取**（`findBalancedJson`）：从首个 `{` 或 `[` 开始，使用深度计数器（depth-tracking）逐字符扫描，正确处理字符串内的转义和嵌套括号，找到匹配的闭合括号后提取子串
4. **贪婪正则回退**（bg-38）：正则 `/(?:\{[\s\S]*\}|\[[\s\S]*\])/` 同时匹配 `{...}` 和 `[...]`（顶层数组支持）
5. 每步提取后均尝试 `JSON.parse()`，且要求 `typeof parsed === "object" && parsed !== null`（拒绝纯量如数字）
6. 全部失败时返回 `null`（不抛异常，前端降级处理）
...
| 502 | （其他错误码） | 上游 Provider 调用失败（含 AI 提取/筛选失败） |
| 503 | （搜索服务不可用） | `/search-with-terms`：未配置搜索 API Key |
...
`updateWorkflowState()` 更新 Zustand 状态后，异步调用 `updateCase()` 写入 IndexedDB（fire-and-forget，`.catch()` 捕获），确保工作流状态在页面刷新后不丢失。写入失败时仅 `console.error`，不阻塞 UI。
...
| `search-references` (步骤1) / `/extract-search-terms` | claimText | 4000 字符 | `String.slice(0, 4000)` | 提取检索关键词（两步式 Step 1） |
| `search-references` (步骤1) / `/extract-search-terms` | queries | 最多 5 条 | `Array.slice(0, 5)` | 每条 2-4 个词；后处理过滤：长度 ≥ 3、排除 markdown fence 和 JSON 开头 |
| `/search-with-terms` (步骤2b) | claimText | 2000 字符 | `String.slice(0, 2000)` | 筛选和排序（两步式 Step 2） |
| `/search-with-terms` (步骤2b) | candidates | 最多 N 条 | `Array.slice(0, maxResults)` | 默认 5，上限 10 |
| `/search-with-terms` (步骤2a EPO) | translations | 与输入等长 | — | 仅 searchProviderId === "epo" 且含中文时触发 |
...
| `/extract-search-terms` (nf-7) | `[{ role: "user", content: prompt }]` | 两步式检索 Step 1：仅提取检索词 |
| `/search-with-terms` (nf-7) | `[{ role: "user", content: prompt }]` | 两步式检索 Step 2：含 EPO 翻译（条件触发）+ 结果筛选 |
...
模块 3（文档分类 / classify-documents）
    ...
    │
    └──→ 模块 15（AI 辅助检索）
              ├── Step 1: /extract-search-terms → 检索词（用户可编辑）
              └── Step 2: /search-with-terms → 可选检索结果 + SearchSession 持久化
```
...
## 27. 附录 G：Coze.cn 移植 Skill 架构指导

本附录指导如何将本文档的 18 个业务模块映射为 Coze.cn 平台的独立 Skill（智能体节点）。每个 Skill 可独立调用，同时通过前驱 Skill 产出的数据变量实现串联。

### Skill 拆分原则

1. **每个模块 = 1 个 Skill**：18 个业务模块对应 18 个 Skill
2. **每个 Skill 可独立运行**：用户可直接输入文件（如审查意见通知书），调用单个 Skill（如"审查意见解析"）获得结构化输出
3. **前驱 Skill 自动触发**：当 Skill 所需的输入数据不存在时，自动调用前驱 Skill 生成
4. **数据通过变量传递**：Skill 间通过 Coze.cn 的变量系统（如 `{{rejectionGrounds}}`、`{{features}}`）传递结构化数据

### Skill 清单与依赖关系

| Skill ID | Skill 名称 | 前驱 Skill | 独立运行时的最小输入 |
|----------|-----------|-----------|-------------------|
| `case-setup` | 案件基线设置 | 无 | 上传申请文件 PDF |
| `document-import` | 文档导入与 OCR | `case-setup` | 上传文件 PDF/DOCX/TXT |
| `classify-documents` | 文档分类 | `document-import` | 已提取文本的文件列表 |
| `opinion-analysis` | 审查意见解析 | `classify-documents` | 审查意见通知书文本（≤12000 字符） |
| `argument-mapping` | 答辩理由映射 | `opinion-analysis` + `classify-documents` | 驳回理由清单 + 意见陈述书文本（≤12000 字符） |
| `interpret` | 文档解读 | `document-import` | 单个文档文本（≤12000 字符） |
| `figure-extract` | 附图提取 | `document-import` | 文档全文 + TextIndex |
| `references` | 文献管理 | `case-setup` | 对比文件 PDF（含公开号、公开日） |
| `claim-chart` | Claim Chart | `document-import` | 权利要求全文 + 说明书文本（≤8000 字符） |
| `novelty` | 新颖性复核 | `claim-chart` + `references` | 技术特征列表 + 单篇对比文件文本（≤8000 字符） |
| `inventive` | 创造性三步法 | `novelty` + `references` | 技术特征列表 + 对比文件摘要（每篇≤500 字符） |
| `defects` | 缺陷复查 | `claim-chart` | 权利要求文本（≤4000 字符）+ 说明书文本（≤8000 字符） |
| `reexam-draft` | 复审意见草稿 | `opinion-analysis` + `argument-mapping` + `novelty` + `inventive` + `defects` | 驳回理由 + 答辩映射 + 新颖性/创造性/缺陷结果 |
| `summary` | 专利申请简述 | `claim-chart` + `novelty` + `inventive` | 已确认特征 + 已审核对照 + 创造性分析 |
| `search-references` | AI 辅助检索 | `claim-chart` | 权利要求文本 + 技术特征列表 |
| `chat` | 模块对话 | 任意模块 | 当前模块上下文 + 用户消息 |
| `translate` | 文档翻译 | `document-import` | 外文文档文本（≤12000 字符） |
| `export` | 导出 | 所有上游模块 | 案件全流程数据 |
...
reexam-draft ← opinion-analysis + argument-mapping + novelty + inventive + defects
...
1. **单条 User Message**：所有 Agent（含 chat）均使用单条 `role: "user"` message 发送，无独立 system role。Coze.cn 创建 Bot 时，可参考 `shared/src/prompts/*.prompt.md` 中的 system/user prompt 拆分素材获得更自然的指令结构
2. **JSON 输出约束**：所有结构化 Agent 的 Prompt 包含完整 JSON Schema 示例，Coze.cn 的"输出格式化"节点应配置对应的 JSON Schema 校验
3. **截断限制**：附录 D 中的截断限制直接影响 token 消耗，Coze.cn 节点配置 maxTokens 时应参考
4. **Provider 配置**：translate Agent 不支持独立 Provider 配置（见模块 18 业务规则），Coze.cn 移植时建议为翻译节点独立配置模型
5. **容错逻辑**：argument-mapping 的 `amendedClaimItemSchema` 使用 `z.union([object, string])` 兼容 AI 返回字符串的降级处理，reexam-draft 的 `supportingEvidenceSchema` 使用 transform 自动降级短引文的 confidence——Coze.cn 的输出格式化需复现此容错