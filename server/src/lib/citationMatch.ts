/**
 * 服务端引用匹配模块 — MIGRATE-010
 * 从前端 citationMatch.ts 迁移到后端
 */
import type { Citation, TextIndex } from "@shared/types/domain.js";
...
export function matchCitation(citation: Citation, index: TextIndex): MatchResult {
  ...
  if (citation.quote && citation.quote.length >= 10) {
    ...
    if (matches.length === 1) {
      ...
      if (!match) return { status: "not-found", confidence: "low" };
